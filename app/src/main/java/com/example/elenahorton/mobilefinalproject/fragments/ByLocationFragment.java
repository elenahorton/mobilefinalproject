...
public class ByLocationFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_bylocation, null);
        parentActivity = (PostsActivity) getActivity();
        RecyclerView recyclerViewPosts = (RecyclerView) rootView.findViewById(R.id.recyclerLocPostList);
        final LinearLayoutManager layoutManager = new LinearLayoutManager(parentActivity);
        layoutManager.setReverseLayout(true);
        layoutManager.setStackFromEnd(true);
        recyclerViewPosts.setLayoutManager(layoutManager);
        recyclerViewPosts.setAdapter(parentActivity.getPostsLocAdapter());
        return rootView;
    }
}
